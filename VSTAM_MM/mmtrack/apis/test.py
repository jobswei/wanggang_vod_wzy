    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            if mode=="vstam":
                batch_input=[]
                for i in data['img'][0]:
                    batch_input.append(dict(image=i))
                ...
                if not isinstance(result,dict):
                    ...
                    for i in range(num_images):
                        img_instances=result[i]['instances']
                        img_instances=img_instances.get_fields()
                        if "pre_masks" in img_instances:
                            temp['det_masks']=img_instances['pre_masks'].permute(1,0,2,3).cpu().numpy()
                        temp['det_bboxes'].append(img_instances['pred_boxes'].tensor.cpu().tolist())
                        temp['det_classes'].append(img_instances['pred_classes'])

                        scores=img_instances['scores'].cpu().tolist()
                        for j in range(len(scores)):
                            temp['det_bboxes'][i][j].append(scores[i])
                        temp['det_bboxes'][i]=np.array(temp['det_bboxes'][i])
                    result=temp    
            else:
...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False,mode=None):
